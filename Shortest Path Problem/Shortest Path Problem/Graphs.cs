...
namespace Shortest_Path_Problem
{
    ...
    public class WeightedGraph : Graph {
        ...
        public Dictionary<(object, object), float> EdgeWeights { get; private set; }
        ...
        public WeightedGraph(HashSet<object> Vertices = null, HashSet<(object, object)> Edges = null, Dictionary<(object, object), float> EdgeWeights = null) {
        ...
        public override void SetGraph(Graph graph) {
            base.SetGraph(graph);
            SetDefaultEdgeWeights();
        }

        public override void AddEdge((object, object) edge) {
            base.AddEdge(edge);
            EdgeWeights[edge] = int.MaxValue;
        }
        ...
        public void SetDefaultEdgeWeights() {
            foreach ((object, object) edge in Edges) {
                UpdateEdgeWeight(edge, float.MaxValue);
            }
        }

        public void SetEdgeWeights(Dictionary<(object, object), float> weights) {
            EdgeWeights = weights;
        }

        public float GetEdgeWeight((object, object) edge) {
            if (EdgeWeights.ContainsKey(edge)) return EdgeWeights[edge];
            return EdgeWeights[(edge.Item2, edge.Item1)];
        }

        public void UpdateEdgeWeight((object, object) edge, float value) {
    ...
    public class StarGraph : WeightedGraph {
        /// <summary>
        ...
        /// </summary>
        public Dictionary<object, float> VertWeights;

        #region Constructors
        public StarGraph() { }

        public StarGraph(HashSet<object> Vertices, HashSet<(object, object)> Edges, Dictionary<(object, object), float> EdgeWeights, Dictionary<object, float> VertWeights) {
            SetGraph(new Graph(Vertices, Edges));
            SetEdgeWeights(EdgeWeights);
            SetVertWeights(VertWeights);
        }

        public StarGraph(Graph graph) {
            SetGraph(graph);
            SetDefaultEdgeWeights();
            SetDefaultVertWeights();
        }

        public StarGraph(WeightedGraph weightedGraph) {
            SetGraph(weightedGraph);
            SetEdgeWeights(weightedGraph.EdgeWeights);
            SetDefaultVertWeights();
        }
        #endregion
        ...
        #region Vert Weight Management

        public void SetDefaultVertWeights() {
            foreach (object vert in Vertices) {
                UpdateVertWeight(vert, 0);
            }
        }

        public void SetVertWeights(Dictionary<object, float> vertWeights) {
            VertWeights = vertWeights;
        }

        public void UpdateVertWeight(object vertex, float value) {
            VertWeights[vertex] = value;
        }

        public float GetVertWeight(object vertex) {
            return VertWeights[vertex];
        }

        #endregion

        #region Pathfinding
        public void AStar(object root, object destination, Dictionary<object, float> vertWeightFromDestination, out Dictionary<object, object> parents, out Dictionary<object, float> distance) {
            //Setup
            HashSet<object> S = new();
            distance = new();
            List<object> Q = new();
